...
// Global variable provided by Chart.js CDN
...
function renderKPIs(data: any) {
  ...
  if (revEl) revEl.textContent = `₹${(data.totalRevenue || 0).toLocaleString()}`;
  if (regEl) regEl.textContent = (data.totalRegistrations || 0).toString();
  if (actEl) actEl.textContent = (data.activeEvents || 0).toString();
}

function renderChart(monthlyData: any[]) {
