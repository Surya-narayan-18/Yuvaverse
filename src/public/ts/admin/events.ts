...
function openRevenueModal(eventId: string, eventTitle: string) {
  ...
  fetch(`/api/admin/events/${eventId}/revenue`, { headers: { Authorization: `Bearer ${token}` } })
    ...
    .then(json => {
      ...
      const d = json.data as { revenue: number, price: number, successRegs: number, successTeams: number, pendingRegs: number, failedRegs: number, pendingTeams: number, failedTeams: number };
