...
(function () {
    ...
    function validate(data) {
        ...
        if (data['name'].length < 2) {
            setError('name', 'Full name must be at least 2 characters.');
            ok = false;
        }
        ...
        if (!data['roleAppliedFor']) {
            setError('roleAppliedFor', 'Please select a role.');
            ok = false;
        }
        if (data['resumeLink'] && !/^https?:\/\//.test(data['resumeLink'])) {
            setError('resumeLink', 'Resume link must be a valid URL.');
            ok = false;
        }
        return ok;
    }
